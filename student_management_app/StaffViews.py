...
from student_management_app.models import CustomUser, Staffs,Question, Courses, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult
...
def staff_add_result2(request):
    return render(request, "staff_template/staff_profile copy.html")    

def staff_add_result3(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method")
        return redirect('staff_add_result3')
    else:
        course = request.POST.get('course')
        question = request.POST.get('qus')
        answer = request.POST.get('ans')
        option_one = request.POST.get('opp1')
        option_two = request.POST.get('opp2')
        option_three = request.POST.get('opp3')
        option_four= request.POST.get('opp4')
        course_obj = Courses.objects.get(id=course)
        qus = Question.objects.create(course = course_obj, question = request.POST.get('qus'), answer = request.POST.get('ans'), 
        option_one = request.POST.get('opp1'),option_two = request.POST.get('opp2'), option_three = request.POST.get('opp3'), option_four= request.POST.get('opp4'))
        return render(request, "staff_template/staff_profile copy.html")  
...
def staff_add_result_save(request):
    if request.method != "POST":
       ...
       # mid_exam_marks = request.POST.get('mid_exam_marks')
        subject_id = request.POST.get('subject')
...
def start_quize(request):
    ...
    return render(request , 'staff_template/qqui.html'  , {'obj' : obj,'question' : question})      

def start_result(request): 
    return render(request ,'staff_template/result.html')       
...
def take_quiz(request , id):
    context = {'id' : id}
    return render(request , 'staff_template/quiz.html'  , context)            

def api_question1(request , id):
    raw_questions = Question.objects.filter(course =id)[:20]
    questions = []
...
def start_exam_view(request,id):
    ...
    questions=Question.objects.filter(course =id)[:20]
    if request.method=='POST':
        pass
    response= render(request,'staff_template/start_exam.html',{'course':course,'questions':questions})
    response.set_cookie('course_id',Courses.id)
    return response    
...
def calculate_marks_view(request):
    if request.COOKIES.get('course_id') is not None:
        ...
        total_marks=0
        questions=Question.objects.filter(course =id)
